...
pub fn fetch_contacts_async(
    ...
    thread::spawn(move || {
        ...
        let all_contacts = plist::parse_contacts_plist(&file_path);
...
pub fn fetch_mails_async(
    page: i32,                    // 页码
    page_size: i32,               // 分页大小
    file_path: String,            // plist的文件路径
    callback: FetchMailsCallback, // 回调函数
) {
    // 创建一个新线程执行异步操作
    thread::spawn(move || {
        // 解析plist文件，获取所有邮件 (XML->Rust)
        let all_mails = plist::parse_mails_plist(&file_path);

        // 调用记录日志的工具函数，显示已加载邮件的数量
        utils::log(&format!("Loaded {} mails from plist", all_mails.len()));

        // 计算当前页面中，首个邮件的位置
        let start = (page * page_size) as usize;

        // 获取当前分页范围内的所有邮件数据
        let page_mails = all_mails
            .into_iter() // 转换为迭代器
            .skip(start) // 跳过当前分页之前的元素
            .take(page_size as usize) // 取指定数量的元素
            .collect::<Vec<_>>(); // 收集为Vec数组

        // 调用序列化函数，将邮件数据编码成 Protobuf 二进制数据
        let pb_data = lark_sdk_pb::serialize_mails(page_mails);
        let len = pb_data.len();
        ...
        // 将指针作为callback的参数传入，使Swift侧能够根据该指针访问到 Protobuf 编码后的邮件数据
        unsafe {
            callback(ptr, len);
        }
    });
